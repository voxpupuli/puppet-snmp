...
when 'RedHat'
  ...
  modified_snmpd_options = '-LS0-4d'
when 'Debian'
  client_package = 'snmp'
  service_config = '/etc/snmp/snmpd.conf'
  service_varconfig = '/var/lib/snmp/snmpd.conf'
  modified_snmpd_options = '-Ls4d -Lf /dev/null -u Debian-snmp -g Debian-snmp -I -smux,mteTrigger,mteTriggerConf -f'
when 'Suse'
  client_package = 'net-snmp'
  service_config = '/etc/snmp/snmpd.conf'
  service_varconfig = '/var/lib/snmp/snmpd.conf'
  modified_snmpd_options = '4d'
end
